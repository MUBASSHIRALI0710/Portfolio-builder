// ============================================
// PORTFOLIO BUILDER - DAY 3 (FINAL WORKING)
...
function renderCanvas() {
  ...
  // Update property editor after rendering
...
function renderComponentByType(comp) {
  ...
  switch (comp.type) {
    case 'header':
      ...
      return `<h${level} style="${style} margin:0.5rem 0;">${comp.content}</h${level}>`;

    case 'paragraph':
      return `<p style="${style} margin:0.5rem 0;">${comp.content}</p>`;

    case 'image':
      return `
                <img src="${comp.content}" alt="${comp.caption || 'Portfolio image'}" style="max-width:100%; border-radius:12px;" onerror="this.src='https://picsum.photos/600/300?random=1'">
...
function showPropertyEditor(index) {
    console.log('showPropertyEditor called for index:', index);
    
    const editorDiv = document.getElementById('propertyEditor');
    if (!editorDiv) {
        console.error('propertyEditor div not found!');
        return;
    }
    ...
    console.log('Editing component type:', comp.type);
    
    // Clear the editor div first
    editorDiv.innerHTML = '';
    ...
    switch(comp.type) {
        ...
        case 'image':
            editorHtml += `
                ...
                <div class="property-row">
                    <img src="${comp.content || ''}" style="max-width:100%; border-radius:12px; max-height:150px; object-fit:cover;" onerror="this.src='https://picsum.photos/200/150?random=1'">
                </div>
            `;
            break;
    ...
    // Common properties for header and paragraph
    if (comp.type === 'header' || comp.type === 'paragraph') {
        editorHtml += `
            <div class="property-row">
    ...
    console.log('Property editor HTML set, length:', editorHtml.length);
...
<body>
  ...
  components.forEach(comp => {
    ...
    switch (comp.type) {
      ...
      case 'image':
        html += `<img src="${comp.content}" alt="${comp.caption || ''}" onerror="this.src='https://picsum.photos/600/300?random=1'">`;
        if (comp.caption) {
